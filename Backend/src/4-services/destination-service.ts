...
class DestinationService {
    ...
    //  public async addLike(like: { destinationId: number; userId: number }): Promise<any> {
    ...
    //       try {
    //         info = await dal.execute<OkPacketParams>(sql, [
    //             like.destinationId,
    //             like.userId
    //         ]);
    //        } catch (err) {
    ...
    //         let newLike;
    //         try {
    //             newLike = await this.getLike(info.insertId);
    //         } catch (err) {
    //             throw new Error("Failed to retrieve like after insertion: " + err.message);
    //         }
